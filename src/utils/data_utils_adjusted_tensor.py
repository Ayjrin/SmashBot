##From: Nathaniel-speiser, pulled by melee-predictor on April 16th
...
def train_game_to_df(game, game_id):
    """
    Turn a game into a df object for study.
    ...
    times = np.array(list(range(0,duration, 1)))
    p1_stocks, p2_stocks = np.zeros(times),np.zeros(times)
    p1_damage, p2_damage = np.zeros(times),np.zeros(times)
    p1_direction, p2_direction = np.zeros(times),np.zeros(times)


    for i, frame_num in enumerate(times):
        #Pre everything
        p1_direction[i] = game.frames[frame_num].ports[port1].leader.pre.direction
        p2_direction[i] = game.frames[frame_num].ports[port2].leader.pre.direction
        ...
        #Calculation for specific variables of interest that can be retrieved live



    ingame_stats = get_ingame_stats(game, times, port1,port2)
    game_id_list = [game_id]*times
    p1_char_list = [p1_char]*times
    p2_char_list = [p2_char]*times
    stage_list = [stage]*times
    winner_list = [winner] * times
    info_df=pd.DataFrame(data = {
            'id'     : game_id_list,
            'p1_char': p1_char_list,
            'p2_char': p2_char_list,
            'stage' : stage_list,
            'frames_elapsed'   : times,
            'p1_direcion' : p1_direction,
            'p2_direcion' : p2_direction,
            'p1_stocks': p1_stocks,
            'p2_stocks': p2_stocks,
            'p1_damage': p1_damage,
            'p2_damage': p2_damage,
